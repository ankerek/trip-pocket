...
> }
> export async function digestStringAsync(algorithm: string, data: string, options?: { encoding?: 'hex' | 'base64' }): Promise<string> {
>   const h = createHash(algorithm);
>   h.update(data, 'utf-8');
...
Find the existing `return <Stack screenOptions={{ headerShown: false }} />;` at the bottom of `RootLayout()` (around line 75) and replace it with explicit `Stack.Screen` entries for every route this plan introduces. The header is *off* globally and *on* explicitly for routes that need it, so the overflow menu / Save / Cancel buttons can render:
